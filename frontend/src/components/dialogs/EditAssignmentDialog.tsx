...
interface EditAssignmentDialogProps {
  ...
  doctors: Doctor[];
  onConfirm: (newDoctorId: string) => void;
  onCancel: () => void;
}
...
export function EditAssignmentDialog({ 
  ...
  const filteredDoctors = doctors.filter(doctor => 
    doctor.id !== assignment.doctor.id && // Exclude current doctor
        ...
        <div className="mb-4">
          <div className="p-3 bg-blue-50 border border-blue-200 rounded mb-4">
            <div className="text-sm text-blue-800">
              <strong>Day {assignment.date}</strong> - Currently assigned to <strong>{assignment.doctor.name}</strong>
            </div>
          </div>
